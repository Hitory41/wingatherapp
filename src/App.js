...
const GiveawayApp = () => {
  ...
  const initializeSupabase = async () => {
    ...
    const tables = ['giveaways', 'user_profiles'];
    ...
    for (const table of tables) {
      try {
        ...
        if (!response.ok) {
          console.log(`Таблица ${table} не существует, пропускаем проверку`);
        }
      } catch (error) {
        console.log(`Таблица ${table} может не существовать, продолжаем`);
      }
    }
  ...
  const saveGiveaway = async (giveaway) => {
    try {
      ...
      if (!response.ok) {
        ...
        console.error('Ошибка создания розыгрыша:', errorText);
  ...
  const updateGiveaway = async (giveaway) => {
    try {
      ...
      const response = await fetch(`${SUPABASE_URL}/rest/v1/giveaways?id=eq.${giveaway.id}`, {
        method: 'PATCH',
        headers: {
          'apikey': SUPABASE_ANON_KEY,
          'Authorization': `Bearer ${SUPABASE_ANON_KEY}`,
      ...
      if (!response.ok) {
        const errorText = await response.text();
        console.error('Ошибка обновления розыгрыша:', errorText);
        throw new Error(`HTTP error! status: ${response.status}`);
      }

      const result = await response.json();
      console.log('Розыгрыш успешно обновлен:', result);
      await loadGiveaways();
      return true;
    } catch (error) {
  ...
  const deleteGiveaway = async (id) => {
    try {
      const response = await fetch(`${SUPABASE_URL}/rest/v1/giveaways?id=eq.${id}`, {
        method: 'DELETE',
        headers: {
          'apikey': SUPABASE_ANON_KEY,
          'Authorization': `Bearer ${SUPABASE_ANON_KEY}`,
      ...
      if (!response.ok) {
        const errorText = await response.text();
        console.error('Ошибка удаления розыгрыша:', errorText);
        throw new Error(`HTTP error! status: ${response.status}`);
      }
  ...
  const saveUserProfile = async (profile) => {
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text();
        console.error('Ошибка сохранения профиля:', errorText);
        throw new Error(`HTTP error! status: ${response.status}`);
      }
  ...
  const handleCreateGiveaway = async () => {
    ...
    if (!formData.title.trim() || !formData.description.trim() || !formData.socialLink.trim() || !formData.endDate) {
      showModal('error', 'Ошибка', 'Пожалуйста, заполните все обязательные поля');
      return;
    }

    if (formData.category === 'Премиум') {
      // Если уже есть активный премиум розыгрыш, сохраняем его как обычный
      if (premiumGiveaway.isActive && premiumGiveaway.title !== 'Создайте премиум розыгрыш') {
        const currentPremium = {
          ...premiumGiveaway,
          id: Date.now().toString(),
          category: 'Обычный'
        };
        const saved = await saveGiveaway(currentPremium);
        if (!saved) {
          showModal('error', 'Ошибка', 'Не удалось сохранить текущий премиум розыгрыш. Попробуйте еще раз.');
          return;
        }
      }

      // Создаем новый премиум розыгрыш
      const newPremium = {
        ...formData,
        id: 'premium',
        participants: 0,
      ...
      setPremiumGiveaway(newPremium);
      showModal('success', 'Успешно', 'Премиум розыгрыш успешно создан!');
    } else {
      ...
      const saved = await saveGiveaway(newGiveaway);
      if (saved) {
        showModal('success', 'Успешно', 'Розыгрыш успешно создан!');
      } else {
  ...
  const handleUpdateGiveaway = async () => {
    ...
    if (formData.category === 'Премиум') {
      // Если редактируем премиум розыгрыш
      if (editingGiveaway && editingGiveaway.id !== 'premium') {
        // Сохраняем текущий премиум как обычный розыгрыш
        const currentPremium = {
          ...premiumGiveaway,
          id: Date.now().toString(),
          category: 'Обычный'
      ...
      setPremiumGiveaway(updatedPremium);
      showModal('success', 'Успешно', 'Премиум розыгрыш успешно обновлен!');
    } else {
      // Обновляем обычный розыгрыш
      const updatedGiveaway = { ...editingGiveaway, ...formData };
      const updated = await updateGiveaway(updatedGiveaway);
      if (updated) {
        showModal('success', 'Успешно', 'Розыгрыш успешно обновлен!');
      } else {
    ...
    showModal(
      ...
      async () => {
        ...
        if (allDeleted) {
          ...
          showModal('error', 'Ошибка', 'Не удалось удалить некоторые демо-розыгрыши. Попробуйте еще раз.');
        }
      },
      hideModal
    );
